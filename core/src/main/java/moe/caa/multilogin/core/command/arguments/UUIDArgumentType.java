...
public class UUIDArgumentType implements ArgumentType<UUID> {
    ...
    private static final DynamicCommandExceptionType dynamicCommandExceptionType = new DynamicCommandExceptionType(value -> new LiteralMessage(
            FormatContent.createContent(FormatContent.FormatEntry.builder().name("value").content(value).build()
            ).format(LanguageHandler.getInstance().getMessage("command_exception_reader_invalid_uuid"))));
